...
export default async function(req){
  ...
  catch(err){await append('dead_letter',{id:crypto.randomUUID(),source:'SAFEPLATE',error:String(err?.message||err),timestamp:new Date().toISOString()},1000);return json({error:String(err?.message||err),mode:'SHADOW'},400)}
}
