...
export default async()=>{
  ...
  return Response.json({service:'VERISCOPE CORE',version:'1.4.0-radar-station',mode:'SHADOW',status,operational,readiness,scoreStatus:'UNVALIDATED_SHADOW_SCORE',autoMergeEnabled:false,humanReviewRequired:true,accessControl,surveillance:{targetMinutes:15,lastSuccessfulCycle:cb.lastSuccess||null,ageMinutes,nextExpectedBy:cb.lastSuccess?new Date(last+15*60_000).toISOString():null},safeplateAdapter:{enabled,state:operational?'TESTED':'CONNECTED_NOT_VERIFIED',circuitBreaker:cb},safeplateFeedback:{enabled:feedbackEnabled,state:feedback?.status||'READY',lastDelivery:feedback?.deliveredAt||null,policy:'HUMAN_APPROVED_ONLY',retryMinutes:15},timestamp:new Date().toISOString()},{headers:{'cache-control':'no-store','x-content-type-options':'nosniff'}});
};
